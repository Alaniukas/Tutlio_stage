...
CREATE INDEX IF NOT EXISTS idx_school_class_groups_org ON public.school_class_groups(organization_id);
CREATE INDEX IF NOT EXISTS idx_school_class_groups_tutor ON public.school_class_groups(tutor_id);
CREATE INDEX IF NOT EXISTS idx_school_class_group_members_student ON public.school_class_group_members(student_id);

ALTER TABLE public.school_contracts DROP CONSTRAINT IF EXISTS school_contracts_class_group_id_fkey;
ALTER TABLE public.school_contracts
...
DROP POLICY IF EXISTS school_class_groups_admin_all ON public.school_class_groups;
...
DROP POLICY IF EXISTS school_class_groups_tutor_select ON public.school_class_groups;
CREATE POLICY school_class_groups_tutor_select ON public.school_class_groups
  FOR SELECT USING (tutor_id = auth.uid());

DROP POLICY IF EXISTS school_class_groups_tutor_insert ON public.school_class_groups;
...
DROP POLICY IF EXISTS school_class_groups_tutor_update ON public.school_class_groups;
CREATE POLICY school_class_groups_tutor_update ON public.school_class_groups
  FOR UPDATE USING (tutor_id = auth.uid())
  WITH CHECK (tutor_id = auth.uid());

DROP POLICY IF EXISTS school_class_group_slots_admin ON public.school_class_group_slots;
...
DROP POLICY IF EXISTS school_class_group_members_admin ON public.school_class_group_members;
...
DROP POLICY IF EXISTS school_class_group_members_tutor_select ON public.school_class_group_members;
...
DROP POLICY IF EXISTS school_class_group_members_student_select ON public.school_class_group_members;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'sessions_class_group_id_fkey'
      AND conrelid = 'public.sessions'::regclass
  ) THEN
    ALTER TABLE public.sessions
      ADD CONSTRAINT sessions_class_group_id_fkey
      FOREIGN KEY (class_group_id) REFERENCES public.school_class_groups(id) ON DELETE SET NULL;
  END IF;
END $$;
...
DROP POLICY IF EXISTS school_monthly_invoices_admin ON public.school_monthly_invoices;
CREATE POLICY school_monthly_invoices_admin ON public.school_monthly_invoices
  FOR ALL USING (public.is_school_admin(organization_id))
  WITH CHECK (public.is_school_admin(organization_id));

DROP POLICY IF EXISTS school_monthly_invoices_student_select ON public.school_monthly_invoices;
...
DROP POLICY IF EXISTS school_lesson_recordings_admin ON public.school_lesson_recordings;
CREATE POLICY school_lesson_recordings_admin ON public.school_lesson_recordings
  FOR ALL USING (public.is_school_admin(organization_id))
  WITH CHECK (public.is_school_admin(organization_id));

DROP POLICY IF EXISTS school_lesson_recordings_tutor_select ON public.school_lesson_recordings;
...
DROP POLICY IF EXISTS school_lesson_recordings_tutor_update ON public.school_lesson_recordings;
CREATE POLICY school_lesson_recordings_tutor_update ON public.school_lesson_recordings
  FOR UPDATE USING (
    session_id IN (SELECT id FROM public.sessions WHERE tutor_id = auth.uid())
  );

DROP POLICY IF EXISTS school_lesson_recording_groups_admin ON public.school_lesson_recording_groups;
...
DROP POLICY IF EXISTS school_lesson_recording_groups_tutor ON public.school_lesson_recording_groups;
...
DROP POLICY IF EXISTS school_lesson_recordings_member_select ON public.school_lesson_recordings;
