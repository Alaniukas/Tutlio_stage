-- Inbox now also recognises 'parent' counterparties so the UI can label them correctly.
-- Order matters: student (linked_user_id) is checked first to keep linked-student-with-parent
-- accounts labelled as students, then parent_profiles, then org admin, else tutor fallback.
...
AS $$
  ...
  parent_info AS (
    ...
    ORDER BY pp.user_id, pp.created_at DESC
  )
  SELECT
    op.conversation_id,
    c.last_message_at,
    op.other_user_id,
    COALESCE(p.full_name, si.full_name, pi.full_name, 'Unknown') AS other_user_name,
    COALESCE(p.email, si.email, pi.email, '') AS other_user_email,
    ...
    CASE
      WHEN EXISTS (
        SELECT 1 FROM public.students s WHERE s.linked_user_id = op.other_user_id
      ) THEN 'student'
      WHEN EXISTS (
        SELECT 1 FROM public.parent_profiles pp2 WHERE pp2.user_id = op.other_user_id
      ) THEN 'parent'
      WHEN EXISTS (
        SELECT 1 FROM public.organization_admins oa WHERE oa.user_id = op.other_user_id
      ) THEN 'org_admin'
      ELSE 'tutor'
    END AS other_party_kind
  ...
  LEFT JOIN parent_info pi ON pi.user_id = op.other_user_id
