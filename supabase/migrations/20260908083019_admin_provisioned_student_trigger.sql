...
DECLARE
  ...
  v_org_id uuid;
BEGIN
  -- Only the trusted admin endpoint sets this server-controlled metadata.
  -- It performs an explicit organization-scoped link without email matching.
  IF new.raw_app_meta_data ? 'provisioned_by_organization' THEN
    RETURN new;
  END IF;
