-- Allow parents to start a conversation with their own child (the linked student user),
-- and the linked student to start a conversation with their parent.
-- Extends get_or_create_conversation relationship validation. Keeps all previous branches intact.
...
BEGIN
  ...
  -- NEW: parent -> their own child (via parent_students -> students.linked_user_id)
  IF NOT v_valid AND EXISTS (
    SELECT 1
    FROM parent_profiles pp
    JOIN parent_students ps ON ps.parent_id = pp.id
  ...
  -- NEW: child (linked student user) -> their parent
  IF NOT v_valid AND EXISTS (
    SELECT 1
    FROM parent_profiles pp
    JOIN parent_students ps ON ps.parent_id = pp.id
