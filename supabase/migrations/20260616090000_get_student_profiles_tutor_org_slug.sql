-- Expose the tutor's organization slug to the student portal so the client can
-- resolve per-organization fee deals (e.g. Proklasė) for the Stripe charge label.
-- Mirrors the existing tutor_organization_entity_type column added previously.
