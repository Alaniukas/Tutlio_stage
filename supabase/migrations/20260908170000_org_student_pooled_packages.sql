...
ALTER TABLE public.lesson_packages
  ADD COLUMN pool_organization_id uuid REFERENCES public.organizations(id),
  ADD COLUMN pool_identity_key text,
  ADD COLUMN pool_student_ids uuid[],
  ADD COLUMN pool_preview_token text,
  ADD COLUMN pool_session_ids uuid[],
  ADD COLUMN pool_email_claimed_at timestamptz;
CREATE UNIQUE INDEX lesson_packages_pool_month_unique
...
RETURNS text LANGUAGE sql IMMUTABLE SET search_path = public AS $$
  SELECT CASE WHEN p_student.linked_user_id IS NOT NULL THEN 'u:' || p_student.linked_user_id::text
...
BEGIN
  ...
  IF v_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM lesson_packages WHERE id=v_id AND pool_preview_token=p_preview_token) THEN
  ...
  VALUES(v_student.tutor_id,p_student_id,NULL,v_total,v_total,0,0,p_unit_price,v_total*p_unit_price,false,'pending',true,'stripe',
    p_period_start,p_period_end,((p_period_end+1)::timestamp AT TIME ZONE 'Europe/Vilnius'),p_org_id,package_student_identity(v_student),p_student_ids,p_preview_token,p_session_ids)
...
BEGIN
  ...
  IF TG_OP='UPDATE' THEN
    IF NEW.pool_organization_id IS DISTINCT FROM OLD.pool_organization_id OR NEW.pool_student_ids IS DISTINCT FROM OLD.pool_student_ids
...
BEGIN
  IF NEW.pool_organization_id IS NOT NULL AND NEW.paid AND NOT OLD.paid THEN
    UPDATE sessions SET lesson_package_id=NEW.id, paid=true, payment_status='paid'
      WHERE id=ANY(NEW.pool_session_ids) AND lesson_package_id IS NULL AND NOT paid
...
CREATE POLICY pooled_package_member_read ON public.lesson_packages FOR SELECT TO authenticated USING (
  pool_organization_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM students s WHERE package_student_identity(s)=pool_identity_key AND s.detached_at IS NULL
      AND s.organization_id=pool_organization_id AND (s.tutor_id=auth.uid() OR s.linked_user_id=auth.uid())
  )
);

-- Invoker security preserves both student and package RLS, including org roles.
CREATE OR REPLACE FUNCTION public.get_pooled_packages_for_student(p_student_id uuid)
RETURNS SETOF public.lesson_packages LANGUAGE sql STABLE SECURITY INVOKER SET search_path=public AS $$
  SELECT p.* FROM lesson_packages p JOIN students s ON s.id=p_student_id
  ...
  WHERE s.detached_at IS NULL AND p.paid AND p.active AND p.payment_status='paid'
    AND p.available_lessons>0 AND p.expires_at>now() ORDER BY p.created_at;
$$;
