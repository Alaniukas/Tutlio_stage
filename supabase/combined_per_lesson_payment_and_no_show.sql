-- Vienas idempotentus SQL skriptas (per-lesson mokinio laukai + no_show_when).
--
-- Kaip naudoti:
-- • Supabase Dashboard → SQL Editor → įklijuok ir paleisk (saugu pakartoti).
-- • Arba lygiavertis turinys jau yra atskirose migracijose:
--   supabase/migrations/20260411120000_student_per_lesson_payment_rules.sql
--   supabase/migrations/20260411140000_session_no_show_when.sql
--   Jei naudoji `supabase db push`, papildomai šio failo paleisti nereikia.

-- ─── Per-student apmokėjimo už pamoką laikas (NULL = paveldėti) ───
...
-- ─── Neatvykimas: kada pažymėta (prieš / per / po) ───
