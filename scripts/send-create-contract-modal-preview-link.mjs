/**
 * Send preview link for create-contract modal to a reviewer.
 *
 * Usage:
 *   node scripts/send-create-contract-modal-preview-link.mjs [email] [previewUrl]
 *
...
const previewUrl =
  process.argv[3] || 'https://www.tutlio.pl/preview-create-contract-modal.html';
...
const bodyHtml = `<p style="font-size:15px;line-height:1.6;color:#374151;margin:0 0 16px;">
  Peržiūrėkite „Nauja sutartis“ modalą su mokinio paieška dropdown'e (fake duomenys, prisijungti nereikia):
</p>
<p style="margin:0 0 8px;">
  <a href="${previewUrl}" style="font-size:16px;font-weight:600;color:#059669;">${previewUrl}</a>
</p>`;
...
if (serviceKey) {
  const res = await fetch(`${appUrl}/api/send-email`, {
    ...
    body: JSON.stringify({
      ...
      locale: 'lt',
      data: {
        subject: 'Tutlio: naujos sutarties modal peržiūra',
        bodyHtml,
      },
    }),
  });
