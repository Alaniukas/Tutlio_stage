...
describe('Hebrew transactional email rendering', () => {
  it('renders the payment reminder in RTL, escaping names and keeping the amount and link', async () => {
    ...
    await handler({
      ...
      body: {
        type: 'payment_reminder', to: 'payer@example.com', locale: 'he', dryRun: true,
        data: { studentName: '<b>דניאל</b>', recipientName: 'הורה', tutorName: 'שרה',
    ...
    expect(payload.html).toContain('<html lang="he" dir="rtl">');
    expect(payload.html).toContain('<body dir="rtl"');
    expect(payload.html).toContain('&lt;b&gt;דניאל&lt;/b&gt;');
    expect(payload.html).not.toContain('<b>דניאל</b>');
    expect(payload.html).toContain('עדיין לא שילם עבור השיעור.');
  ...
  it('gives standalone business welcome and tutor invitations RTL envelopes', async () => {
    await sendEnterpriseWelcomeEmail('admin@example.com', {
      companyName: 'עסק לדוגמה', licenseCount: 3, setupLink: 'https://example.com/setup', locale: 'he',
    });
    expect(sendMock.mock.calls[0][0].html).toContain('<html lang="he" dir="rtl">');
    expect(sendMock.mock.calls[0][0].html).toContain('עסק לדוגמה');
    await sendTutorInviteEmail('tutor@example.com', {
      inviteToken: 'test-invite', orgName: 'עסק לדוגמה', inviteeName: 'שרה',
      inviteeEmail: 'tutor@example.com', origin: 'https://www.tutlio.com', emailLocale: 'he', uiLocale: 'he',
    });
    expect(sendMock.mock.calls[1][0].html).toContain('<html lang="he" dir="rtl">');
    expect(sendMock.mock.calls[1][0].html).toContain('/he/');
  });
});
