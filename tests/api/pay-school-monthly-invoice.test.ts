...
describe('GET /api/pay-school-monthly-invoice', () => {
  ...
  it('creates a Connect checkout for the invoice total and redirects to it', async () => {
    ...
    const [params, options] = stripeMocks.create.mock.calls[0];
    ...
    const {
      ...
      transferToSchoolCents,
    } = schoolInstallmentCheckoutCents(162, 'default');
    expect(charged).toBe(chargeCents);
    expect(charged).toBe(16200);
    expect(applicationFeeCents).toBe(162);
    expect(estimatedStripeFeeCents).toBe(268);
    expect(transferToSchoolCents).toBe(15770);
    expect(params.payment_intent_data.application_fee_amount).toBe(applicationFeeCents);
