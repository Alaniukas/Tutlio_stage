...
describe('Hong Kong payment reminder preview', () => {
  it.each([
    { paymentTiming: 'before_lesson', hours: 24, recipientName: '家長', timingText: '課堂前 24 小時' },
    { paymentTiming: 'after_lesson', hours: 2, recipientName: '<b>Ana</b>', timingText: '課堂後 2 小時' },
    ...
    await handler({
      ...
      body: {
        type: 'payment_reminder', to: 'payer@example.com', locale: 'zh-hk', dryRun: true,
    ...
    // BCP 47 tags are case-insensitive; email wrappers retain the URL slug.
    expect(payload?.html).toMatch(/<html lang="zh-hk" dir="ltr">/i);
    expect(payload?.html).toContain('付款期限');
    expect(payload?.html).toContain(timingText);
    expect(payload?.html).toContain(recipientName === '家長'
      ? '學生 <strong>&lt;b&gt;Ana&lt;/b&gt;</strong> 尚未繳付課堂費用。'
      : '你尚未繳付課堂費用。');
