...
describe('Filipino payment reminder preview', () => {
  it.each([
    { paymentTiming: 'before_lesson', hours: 24, recipientName: 'Magulang', timingText: '24 oras bago ang sesyon' },
    { paymentTiming: 'after_lesson', hours: 2, recipientName: '<b>Ana</b>', timingText: '2 oras pagkatapos ng sesyon' },
    ...
    await handler({
      ...
      body: {
        type: 'payment_reminder', to: 'payer@example.com', locale: 'fil', dryRun: true,
    ...
    expect(payload?.html).toContain('<html lang="fil" dir="ltr">');
    expect(payload?.html).toContain('Takdang oras ng pagbabayad');
    expect(payload?.html).toContain(timingText);
    expect(payload?.html).toContain(recipientName === 'Magulang'
      ? 'Hindi pa nababayaran ng estudyanteng si <strong>&lt;b&gt;Ana&lt;/b&gt;</strong> ang sesyon.'
      : 'Hindi mo pa nababayaran ang sesyon.');
