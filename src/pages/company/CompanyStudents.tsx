...
import { Plus, Trash2, User, Users, UserMinus, Mail, Phone, GraduationCap, CheckCircle, XCircle, Sparkles, Package, Loader2, FileText, Search, Euro, Clock, MessageSquare, Archive, ArchiveRestore, Download, AlertCircle, Ban, Pencil } from 'lucide-react';
...
import {
  ...
  type ProKlaseClientRosterFilter,
} from '@/lib/proKlaseClientRoster';
import { monthlyPackageErrorMessage } from '@/lib/monthlyPackageUi';
...
export default function CompanyStudents() {
  ...
  const [pkClientFilter, setPkClientFilter] = useState<ProKlaseClientRosterFilter>('all');
  ...
  const filteredGroups = useMemo(() => {
    ...
    if (proKlaseAdminUi && pkClientFilter !== 'all') {
      groups = groups.filter((g) => matchesProKlaseClientRosterFilter(g, pkClientFilter));
    }
    return groups;
  }, [
    groupedStudents,
  ...
  const pkRosterCounts = useMemo(
    () => countProKlaseClientRoster(groupedStudents),
    [groupedStudents],
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(monthlyPackageErrorMessage(result.error || t('common.error'), t));
        }
  ...
  const handleSaveMonthlyPackage = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(monthlyPackageErrorMessage(result.error || t('compStu.errorSendingPackage'), t));
      }
        ...
        <div className="mb-6 space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              ...
              {proKlaseAdminUi ? t('compStu.pkTitle') : t('compStu.title')}
              ...
              >
                <DialogTrigger asChild>
                  <Button className="gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700">
                    ...
                    {proKlaseAdminUi ? t('compStu.pkAddStudent') : t('compStu.addStudent')}
                  </Button>
                </DialogTrigger>
          ...
          {proKlaseAdminUi && !showTrashBin ? (
            <div className="flex flex-wrap items-center gap-2">
              {([
                { id: 'all' as const, label: t('compStu.pkAllClients'), count: pkRosterCounts.all, icon: Users, active: 'bg-indigo-50 text-indigo-800 ring-indigo-200', idle: 'bg-white text-indigo-700 ring-indigo-100' },
                { id: 'active' as const, label: t('compStu.pkActiveClients'), count: pkRosterCounts.active, icon: CheckCircle, active: 'bg-emerald-50 text-emerald-800 ring-emerald-200', idle: 'bg-white text-emerald-700 ring-emerald-100' },
                { id: 'inactive' as const, label: t('compStu.pkInactiveClients'), count: pkRosterCounts.inactive, icon: Clock, active: 'bg-amber-50 text-amber-800 ring-amber-200', idle: 'bg-white text-amber-700 ring-amber-100' },
                { id: 'unassigned' as const, label: t('compStu.pkUnassignedTutor'), count: pkRosterCounts.unassigned, icon: UserMinus, active: 'bg-violet-50 text-violet-800 ring-violet-200', idle: 'bg-white text-violet-700 ring-violet-100' },
              ]).map((chip) => {
                const selected = pkClientFilter === chip.id;
                const Icon = chip.icon;
                return (
                  <button
                    key={chip.id}
                    type="button"
                    onClick={() => setPkClientFilter((current) => toggleProKlaseClientRosterFilter(current, chip.id))}
                    className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-medium ring-1 transition-colors ${
                  ...
                  >
                    ...
                    {chip.label}
                    <span className={`rounded-full px-1.5 py-0.5 text-xs font-semibold ${selected ? 'bg-white/80' : 'bg-gray-50 text-gray-600'}`}>
                      {chip.count}
                    </span>
                  </button>
                );
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2">
            ...
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input
                type="search"
                placeholder={proKlaseAdminUi ? t('compStu.pkSearchPlaceholder') : t('compStu.searchPlaceholder')}
                value={studentSearch}
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {student.linked_user_id ? (
                            <span className="text-[11px] text-green-700 bg-green-50 border border-green-200 rounded-md px-1.5 py-0.5 flex-shrink-0">
                              {t(proKlaseAdminUi ? 'compStu.pkAccountConfirmed' : 'compStu.connected')}
                            </span>
                          ) : (
                            <span className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-1.5 py-0.5 flex-shrink-0">
                              {t(proKlaseAdminUi ? 'compStu.pkAccountUnconfirmed' : 'compStu.notConnected')}
                            </span>
                          )}
                        </div>
                        {needsPackage && (
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full table-fixed min-w-[920px]">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    <th className="w-[28%] px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{proKlaseAdminUi ? t('compStu.pkThClient') : t('compStu.thStudent')}</th>
                          ...
                          <div className="flex items-start gap-3 min-w-0">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-1 flex flex-wrap items-center gap-1">
                                {student.linked_user_id ? (
                                  <span className="text-[11px] text-green-700 bg-green-50 border border-green-200 rounded-md px-2 py-0.5">{t(proKlaseAdminUi ? 'compStu.pkAccountConfirmed' : 'compStu.connected')}</span>
                                ) : (
                                  <span className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-2 py-0.5">{t(proKlaseAdminUi ? 'compStu.pkAccountUnconfirmed' : 'compStu.notConnected')}</span>
                                )}
                                {needsPackage && (
                        ...
                        <div className="space-y-2 rounded-xl border border-violet-200 bg-white/70 p-3" aria-live="polite">
                          ...
                          {monthlyPreview ? (
                            <>
                              ...
                              <p className="text-[11px] text-violet-600">{t('package.monthlyUnpaidHint')}</p>
                              <p className="text-xs">
