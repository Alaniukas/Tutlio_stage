...
export default function CompanyWaitlist() {
  ...
  const { hideWaitlist: hideWaitlistResolved, resolved } = useHideWaitlist({
    failClosedWhileLoading: true,
  });
  const hideWaitlist =
    hideWaitlistResolved || isWaitlistHiddenForOrg(cachedOrgId);

  // Never mount waitlist UI for Pro Klasė / disable_waitlist orgs.
  if (hideWaitlist) {
    return <Navigate to={`${orgBasePath}/sessions`} replace />;
  }
  if (!resolved) {
    return (
  ...
  return <CompanyWaitlistLoaded orgBasePath={orgBasePath} />;
}

function CompanyWaitlistLoaded({ orgBasePath }: { orgBasePath: '/school' | '/company' }) {
  const { t } = useTranslation();
