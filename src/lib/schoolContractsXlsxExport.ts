...
import type { SchoolContractExportRow } from './schoolContractsExport';
import { schoolContractsTableData } from './schoolContractsExport';
...
function addContractsSheet(
  workbook: ExcelJS.Workbook,
  rows: SchoolContractExportRow[],
  t: (key: string) => string,
  orgName?: string,
) {
  const { headers, body } = schoolContractsTableData(rows, t);
  const ws = workbook.addWorksheet(t('school.contractExportSheet'), {
  ...
  if (orgName) {
    ...
    subtitleCell.value = t('school.contractExportTitle');
  ...
  body.forEach((row, idx) => {
    ...
    row.forEach((val, colIdx) => {
      ...
      cell.value = val;
      if (colIdx === 7 && typeof val === 'number') {
        cell.numFmt = EURO_FMT;
      }
      cell.alignment = { vertical: 'middle', wrapText: colIdx === 6 };
...
export async function downloadSchoolContractsXlsx(
  rows: SchoolContractExportRow[],
  t: (key: string) => string,
  filename: string,
  orgName?: string,
  ...
  addContractsSheet(workbook, rows, t, orgName);
