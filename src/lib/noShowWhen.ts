...
/** Removes only the exact audit line generated by buildNoShowSessionPatch. */
export function removeGeneratedNoShowTutorComment(
  existingTutorComment: string | null | undefined,
): string | null {
  const kept = (existingTutorComment || '')
    .split('\n')
    .filter((line) => !/^Mokinys neatvyko \((prieš pamoką|pamokos metu|po pamokos)\)\.$/.test(line.trim()))
    .join('\n')
    .trim();
  return kept || null;
}
