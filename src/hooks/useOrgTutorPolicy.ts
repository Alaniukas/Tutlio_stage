...
export function useOrgTutorPolicy(): OrgTutorPolicy {
    ...
    const run = async () => {
      ...
      let effectiveCommissionPercent: number | null =
        profile && typeof (profile as any).company_commission_percent === 'number'
          ...
          : side && typeof side.company_commission_percent === 'number'
            ? Number(side.company_commission_percent)
            : null;
      let effectiveHasActiveLicense: boolean | null =
        typeof (profile as any)?.has_active_license === 'boolean'
          ...
          : typeof side?.has_active_license === 'boolean'
            ? Boolean(side.has_active_license)
            : null;
      ...
      const prof: { company_commission_percent?: number | null; has_active_license?: boolean | null } = {
  ...
  }, [user?.id, profile?.organization_id, profile?.id]);
