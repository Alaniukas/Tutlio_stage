          ...
          {isSchools ? (
            ...
            <div ref={loginRef} className="relative">
              <Button
                ...
                className="rounded-xl px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold tracking-wide shadow-md shadow-indigo-200 gap-2"
              >
                {t('common.login')}
                <AppWindow className="w-4 h-4" />
              </Button>
              ...
              {loginOpen && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50">
                  <Link
                    to="/login"
                    onClick={() => setLoginOpen(false)}
                    className="block px-3 py-2 text-sm hover:bg-gray-50 text-gray-800"
                  >
                    {locale === 'lt' ? 'Prisijungti kaip korepetitorius' : 'Login as tutor'}
                  </Link>
                  <Link
                    to="/school/login"
                    onClick={() => setLoginOpen(false)}
                    className="block px-3 py-2 text-sm hover:bg-gray-50 text-gray-800"
                  >
                    {locale === 'lt' ? 'Prisijungti kaip mokykla' : 'Login as school'}
                  </Link>
                </div>
              )}
            </div>
