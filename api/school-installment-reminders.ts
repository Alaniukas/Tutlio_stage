...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const installmentSelect =
    'id, contract_id, installment_number, amount, due_date, payment_status, reminder_3d_sent_at, reminder_1d_sent_at, contract:school_contracts(id, student_id, organization_id, signing_status, archived_at, terminated_at, suspension_started_at, suspension_until, suspension_resumed_at, annual_fee, additional_fee_amount, additional_fee_purpose, student:students(full_name, email, payer_email, payer_name), org:organizations(name, email, features, stripe_account_id, stripe_onboarding_complete))';
  ...
  for (const inst of installments as any[]) {
    ...
    if (alreadySent || inst.payment_status !== 'pending' || inst.contract?.archived_at
      || inst.contract?.terminated_at || isSchoolContractSuspended(inst.contract || {})) continue;
