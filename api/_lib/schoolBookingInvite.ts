...
export async function sendNextPendingInstallmentAfterSplitFeePaid(
  ...
  const { data: contract } = await supabase
    ...
    .select(
      'id, organization_id, signing_status, terminated_at, suspension_started_at, suspension_until, suspension_resumed_at, annual_fee, additional_fee_amount, additional_fee_purpose, student:students(full_name, email, payer_email, payer_name), organizations(name, email, features)',
    )
    .eq('id', contractId)
    .maybeSingle();

  if (!contract) return false;
  if (schoolContractBlocksService(contract as any)) return false;
