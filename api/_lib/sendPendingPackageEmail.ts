...
export async function sendPendingPackagePaymentEmail(args: SendArgs): Promise<{ ok: true } | { ok: false; status: number; error: string; details?: string }> {
  ...
  const { data: pkg, error: pkgErr } = await supabase
    ...
    .single();
  if (pkgErr || !pkg) {
    return { ok: false, status: 404, error: 'Paketas nerastas', details: pkgErr?.message };
  }

  const tutor = pkg.profiles as any;
  if (tutor?.organization_id !== organizationId) {
    return { ok: false, status: 403, error: 'Package belongs to another organization' };
  }
  if (pkg.paid || pkg.payment_status === 'paid') {
    return { ok: false, status: 400, error: 'Paketas jau apmokėtas' };
  }
  ...
  if (!toEmail) return { ok: false, status: 400, error: 'Mokinys neturi el. pašto adreso' };
  ...
  if (!emailRes.ok) {
    ...
    return { ok: false, status: 502, error: 'Nepavyko išsiųsti laiško', details: txt || String(emailRes.status) };
  }
  return { ok: true };
}
