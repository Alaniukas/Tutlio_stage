...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { base64, contentType, orgId } = body || {};

  if (!base64 || !contentType) return res.status(400).json({ error: 'base64 and contentType required' });
  if (!ALLOWED_TYPES[contentType]) return res.status(400).json({ error: `Unsupported type: ${contentType}. Allowed: jpg, png, webp, svg` });
  if (!orgId) return res.status(400).json({ error: 'orgId required' });

  const buffer = Buffer.from(base64, 'base64');
  if (buffer.length > MAX_SIZE) return res.status(400).json({ error: 'File too large (max 2 MB)' });

  const ext = ALLOWED_TYPES[contentType];
  const path = `org-logos/${orgId}/${randomUUID()}${ext}`;
