...
> **Update 2026-09-13.** All 36 released locales are published on the marketing, schools, public-page and blog surfaces. Blog generation uses Gemini 3.8 Flash with independent locale SEO briefs covering search intent, terminology, examples, units/currency, headline conventions, localized internal links, and prohibited translation artifacts. The editorial gate also requires a defensible thesis, genuine information gain, claim safety, locally plausible examples, useful answer-first structure, and non-formulaic titles; it refuses duplicate keyword URLs and preserves native-script slugs. A post is not published until all 36 locale articles pass the automated quality gate. Covers use a topic-specific visual concept and one of 12 controlled editorial art directions instead of a reusable SaaS-icon template, and crawlable covers are included in the image sitemap. The matching blog-column migration must be applied before the website build is deployed. Legal pages remain on the reviewed legacy 13. The historical sections below describe the earlier gated state.
...
The arrays are independent. The UI and blog-schema lists contain all 36, while legal, localized-asset and platform-copy lists retain their reviewed surface-specific cohorts. Releasing the blog does not release English-fallback legal policies or request nonexistent images. `LEGACY_LOCALES` and the deprecated `TRANSLATED_LOCALES` alias remain historical baselines, not launch switches.
